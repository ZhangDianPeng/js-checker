...
let createTree = function (name, data) {
    ...
    if (['Obj', 'Or', 'Default', 'Value', 'Optional', 'Arr', 'OrVal', 'Map', 'Extend', 'OrValType', 'CFn'].includes(type)) {
                ...
                attribute = [
                    {
                        ...
                        type: 'fnInput',
                        description: '输入参数检查',
                        attribute: attribute.input.map(attr => createTree(void 0, attr))
                    },
                    Object.assign(createTree(void 0, attribute.output), {name: 'output'})
                ];
                return {
                    name, type: 'Obj', description, attribute
                }
...
let getRenderData = function (type, name, description, attribute, isRoot = true) {
    ...
    switch (type) {
        ...
        case 'Map':
            ...
            if (attribute) {
                let keyTemp = getRenderData(mapKey.type, mapKey.name, mapKey.description, mapKey.attribute, false);
                let valueTemp = getRenderData(mapValue.type, mapValue.name, mapValue.description, mapValue.attribute, false);
        ...
        case 'fnInput':
            ...
            template = `
                <div class="or ${isRoot ? 'unfold': 'fold'}" fold-id=${FOLDID++}>
                    ${ name ? `<span class="field-name">${name}:</span>` : '' }
                    ${ type != 'fnInput' ? `<span class="C-type">${type}</span>`: ''}
                    ${ description ? `<span class="description">${description}</span>`:'' }
                    ${Mark`[`}
                    ${ attribute ? `
                    <div class="attribute">
                    ...
                    return `
                            <div class="basic ${hasComplex ? 'fold':''}" fold-id=${FOLDID++}>
                                ${idx}: ${ innerHTML(getRenderData(a.type, a.name, a.description, a.attribute, false)) }
                            </div>
                        `;
                }).join('\n') }
                    </div>
...
let getHtml = (showData, htmlPath) => {
    htmlPath = htmlPath || path.resolve(__dirname, './test.html');
