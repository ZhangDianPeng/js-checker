...
t.Any = id.judge(v => !_.isNil(v), 'is Null').named('Any');

t.Num = t.Any.judge(v => !isNaN(v) && v !== '', 'is not a Num').match(Number).named('Num');

t.Str = t.Any.match(String).named('Str');

t.Fn = t.Any.judge(_.isFunction, 'is not a function').named('Fn');

t.Json = t.Any.judge(_.isObject, 'is not an Obj').named('Json');

t.Obj = t.Json.judge(v => !_.isArray(v), 'is not an Obj').named('TObj');

t.Arr = t.Json.judge(_.isArray, 'is not an Arr').named('TArr');
...
c.Extend = function () {
    ...
    Types.forEach(t => {
        ...
        if(name === 'Obj'){
            ...
            throw new Err(attribute, `Extend不支持${name}类型`);
        }
    });
...
['Val', 'Or', 'Optional', 'Default', 'Arr', 'Obj', 'OrVal', 'OrValType', 'ValType', 'ValSet', 'ValConvert', 'ObjConvert', 'Map', 'Extend']
