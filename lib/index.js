...
let getOrValType = (arr, type) => {
    return c.OrVal.apply(null, arr.map(type)).match(value => type(value)).named('OrValType').attributed({values: arr.map(type), type: type.show});
};
...
c.Str = (value) => getOrValType([value], t.Str).originTyped('c.Str');
c.Num = (value) => getOrValType([value], t.Num);
c.OrStr = function(){
